      ...
      <div className="flex items-start justify-between gap-4">
        <div className="flex-1">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:gap-4">
            ...
            {type && (
              <span
                className={`h-7 w-fit rounded-md bg-[color:var(--color-primary)]/10 px-3 py-1 text-sm font-medium backdrop-blur-[80px] ${type === 'New 🔥' ? 'animate-[var(--animate-blink)] text-[color:var(--color-tag)]' : 'text-[color:var(--color-accent)]'}`}>
                {type}
              </span>
            )}
          </div>
        ...
        <figure className="flex justify-end overflow-hidden">
          <Image
            ...
            height={90}
            alt={`${title} cover`}
            className="h-[90px] w-[160px] rounded-lg object-cover transition-transform duration-300 group-hover:scale-105"
          />
        </figure>
      </div>
